...
const ReviewList = () => {
    ...
    async function getList(){
        const startDate = moment(moment().toDate()).add(-1,"months").format('YYYYMMDD');
        const endDate = moment(moment().toDate()).format('YYYYMMDD');
        const { status, data } = await getGoodsReviewListByProducerNo({stars:null, startDate:startDate, endDate:endDate});
        let items = []
            ...
            </div>
            {
                data.map((item, index) =>
                    <>
                        ...
                        {
                            item.goodsReviewImages.length > 0 && (
                                <Flex overflow={'auto'} mt={16} custom={`
                            & > img {
                                margin-right: 8px;                    
                            }
                            & > img:last-child {
                        ...
                        `}>
                                    {
                                        item.goodsReviewImages.map((image, index) =>
                                            <Img
                                                cursor={1}
                                                ...
                                                alt={image.imageNm}
                                                onClick={openImageViewer.bind(this, item.goodsReviewImages, index)}
                                            />
                                        )
                                    }
                                </Flex>
