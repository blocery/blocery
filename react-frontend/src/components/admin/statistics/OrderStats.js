...
export default class OrderStats extends Component {
    ...
    getGoodsExcelData = () => {
        const columns = [
            '번호', '카테고리','상품번호','상품명', '농장명','신용수량', 'BLCT수량', '카드BLCT수량', '수량합계',
            '신용카드', 'BLCT', '카드BLCT(카드)', '카드BLCT(BLCT)', '매출합계', '정산금액', '판매지원금'
        ]
        //필터링 된 데이터에서 sortedData._original 로 접근하여 그리드에 바인딩 원본 값을 가져옴
        const data = this.state.data.goodsStats.map((item ,index)=> {
            return [
                item.index, item.itemName, item.goodsNo, item.goodsNm, item.producerFarmNm, item.countCard, item.countBlct, item.countCardBlct, item.countSum,
                item.card,item.blct, item.cardBlctByCard, item.cardBlct - item.cardBlctByCard, item.sum, item.simplePayoutAmount, item.saleSupportPrice
            ]
        })
                            ...
                            <>
                                ...
                                <Table bordered>
                                    <tr>
                                        ...
                                        <td width="400px" colSpan="4" bgcolor="#F3F3F3" align="center" valign="middle" > 구매건수 </td>
                            ...
                            <>
                                ...
                                <Table bordered>
                                    <tr>
                                        ...
                                        <td width="200px" colSpan="4" bgcolor="#F3F3F3" align="center" valign="middle" > 구매건수 </td>
