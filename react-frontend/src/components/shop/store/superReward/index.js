...
import {getEventNoByType} from '~/lib/shopApi'
...
const SuperReward = function(props){
    ...
    async function getData(){
        const {data} = await getSuperRewardList(isForceUpdate(props.history), abortControllerRef.current.signal)
        return data
    }
    ...
    async function setMainHtml() {
        ...
        //설정되어 있는 이벤트 번호 가져오기
        const {data: eventNo} = await getEventNoByType('superReward', isForceUpdate(props.history))
