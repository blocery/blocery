...
import {getCommunityGoodsReviewList} from '~/lib/shopApi'
...
const CmGoodsReviewMain = withRouter(({history}) => {
    ...
    const fetchMoreData =  async (isNewSearch, photoChecked) => {
        ...
        let params = { onlyPhotoReview: photoChecked, isPaging: true, limit: 5 }
        ...
        const {data} = await getCommunityGoodsReviewList(params)
    ...
    return (
        <Div>
            ...
            <BackNavigation showShopRightIcons>실시간 리뷰</BackNavigation>
                        ...
                        <ListSpace>
                            {
                                goodsReviewList.map((goodsReview, index) =>
                                    <Div key={`goodsReview${index}`} bc={'light'} bt={0} bl={0} br={0}>
                                        <GoodsReviewCard data={goodsReview} showGoodsNm />
                                    </Div>
                                )
                            }
                        </ListSpace>
...
export default CmGoodsReviewMain;