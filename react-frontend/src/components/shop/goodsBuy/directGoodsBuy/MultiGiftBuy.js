...
export default class MultiGiftBuy extends Component {
    ...
    async componentDidMount() {
        ...
        const optionIndex = params.get('optionIndex')
    ...
    createProducerOrderList = (goods, producer) => {
        const orderList = this.state.excelUploadData.map((receiverInfo, idx) => {
            ...
            order.optionIndex = this.state.optionIndex;
            order.optionName = goods.options[this.state.optionIndex].optionName;
            ...
            //주문가격
            order.orderPrice = MathUtil.multipliedBy(goods.currentPrice,order.orderCnt) + order.deliveryFee ;
            // order.orderPrice = (goods.currentPrice * goods.orderCnt) + order.deliveryFee ;
            order.cardPrice = MathUtil.multipliedBy(goods.currentPrice,goods.orderCnt) + order.deliveryFee ; //202003, blct결제일 경우 0, cardBlct일때는 blctToken제외금액으로 세팅해야함..

            //할인가격도 저장을 해야할지????
            //order.discountFee = (orderInfo.consumerPrice * orderInfo.orderCnt) - (orderInfo.currentPrice * orderInfo.orderCnt);

            //주문가격BLCT
            order.blctToken = ComUtil.roundDown(MathUtil.dividedBy(order.orderPrice,this.blctToWon), 2)
    ...
    handleExcelData = async (jsonData) => {
        ...
        orderListForSaving.map(order => {
            ...
            //총 가격 등 계산 (주문그룹정보)
            g_totalCurrentPrice += MathUtil.multipliedBy(order.currentPrice,order.orderCnt);
    ...
    getSummary = (producer, orderList, goods) => {
        ...
        orderList.map(order => {
            ...
            const goodsPrice = MathUtil.multipliedBy(goods.currentPrice,order.orderCnt)
    ...
    orderValidate = async (orderGroup, orderList) => {
        ...
        if (!chk) {
            toast.error('상품 재고가 부족합니다. 상품재고:' + goods.remainedCnt + '개');
            return false;
        }
        this.setState({
            orders: orderList
        ...
        if(payMethod === "blct"){
            ...
            if(payBlctBalance <= 0){
                toast.warn('보유한 BLCT가 0입니다.');
                this.setState({modal:false})
                return false;
            }
            if(payBlctBalance < payBlct){
                toast.warn('보유한 BLCT가 구매하기에 부족합니다.');
                this.setState({modal:false})
                return false;
            }
        }
    ...
    getOrderSubGroupParam = () => {
        const state = Object.assign({}, this.state)
        const orderSubGroups = []; // dealGodds에서는 subGroup이 1개뿐일듯. 하지만 api에 리스트 넘겨야 함.

        state.orders.forEach(order => {
            const subGroup = {};
            subGroup.deliveryFee = order.deliveryFee;
            // subGroup.deliveryFee = state.orderGroup.totalDeliveryFee;
            subGroup.payMethod = state.selectedPayMethod;

            if (state.selectedPayMethod === 'blct') {
                subGroup.deliveryCardPrice = 0;
                subGroup.deliveryBlctToken = parseFloat(MathUtil.dividedBy(state.orderGroup.totalDeliveryFee,state.blctToWon).toFixed(3))
            } else { // card
                subGroup.deliveryBlctToken = 0;
                subGroup.deliveryCardPrice = state.orderGroup.totalDeliveryFee;
            }
    ...
    modalToggleOk = async () => {
        ...
        if(remain_chk === true){
            ...
            if(payMethod === "blct"){
                ...
                console.log('buyingOrderList 건수:' + buyingOrderList.length);
                const orderSubGroupList = this.getOrderSubGroupParam()
                ...
                let {orderGroup:tmp_OrderGroup, orderDetailList:tmp_OrderList, orderSubGroupList:tmp_orderSubGroupList} = returnedOrders;
                ...
                await this.buyBLCTGoods(tmp_OrderGroup,tmp_OrderList, tmp_orderSubGroupList);
    ...
    buyBLCTGoods = async (tmpOrderGroup,tmpOrderList,tmpOrderSubGroupList) => {
        ...
        let orderSubGroupList = Object.assign([], tmpOrderSubGroupList)
        ...
        orderSubGroupList.forEach(orderSubGroup => {
            ...
            orderListPrice += orderSubGroup.deliveryFee;
        })
        ...
        if (!result || result.resCode !== 0) {
            toast.dismiss();
            toast.warn('상품 구매에 실패 하였습니다. 다시 구매해주세요.');
        } else {
            //구매완료페이지로 이동
    ...
    render() {
        ...
        return (
            <Fragment>
                ...
                <BackNavigation onBackClick={() => Webview.closePopup(false)}>선물하기(여러명)</BackNavigation>
                                        ...
                                        this.state.orders.map((item, index) => {
                                            return(
                                               <tr>
                                                   ...
                                                   <td>{item.orderCnt}</td>
                                                   <td>{ComUtil.addCommas(MathUtil.multipliedBy(item.currentPrice,item.orderCnt))}</td>
                                                   <td>{ComUtil.addCommas(item.deliveryFee)}</td>
                                                   <td>{ComUtil.addCommas(MathUtil.multipliedBy(item.currentPrice,item.orderCnt)+item.deliveryFee)}</td>
                                               </tr>
                                            )
                                        })
                                    }
                ...
                <ItemHeader>
                    <div>결제방법</div>
                    <div><span className={Css.textGray}>보유BLY </span> <span>{ComUtil.addCommas(ComUtil.roundDown(this.state.tokenBalance, 2))}BLY({ComUtil.addCommas((MathUtil.multipliedBy(this.state.tokenBalance,this.state.blctToWon)).toFixed(0))}원)</span></div>
                </ItemHeader>
                <ItemPayMethodBody>
                    {
                        this.state.isExcelUploadFileData ?
                ...
                <ItemDefaultBody>
                    <div className={Css.row}>
                        <div>총 상품가격</div>
                        <div>{ComUtil.addCommas(MathUtil.minusBy(this.state.orderGroup.totalOrderPrice,this.state.orderGroup.totalDeliveryFee))}원</div>
                    </div>
                    <div className={Css.row}>
                        <div>총 배송비</div>
