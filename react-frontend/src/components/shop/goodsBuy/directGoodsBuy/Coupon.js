...
//     }
// `
const Coupon = ({goods, blctToWon, onChange,  totalBlyAmount, totalOrderPrice, history}) => {
    ...
    const getUsableCoupon = async () => {
        ...
        console.log('<Coupon: totalOrderPrice:' + totalOrderPrice)
        ...
        if (data) {
            const couponList = data.map(item => ({
                ...
                label: (
                    item.prodGoodsProducerNo ? (
                        ...
                        <Div>
                            ...
                            {(!item.couponType || item.couponType !== 'deliveryCoupon') && //배송비쿠폰은 금액제외 2208
                                <Div>
                ...
                isDisabled: (item.minOrderBlyAmount > totalBlyAmount || item.couponBlyAmount > totalBlyAmount ||  (item.minGoodsPrice && totalOrderPrice < item.minGoodsPrice  ) )|| item.prodGoodsProducerNo,
